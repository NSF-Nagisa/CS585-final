...
import random
from lib.models.layers.layers import InvBottleneck, convbnrelu, SepConv2d, Block
...
class LitePose(nn.Module):
    def __init__(self, cfg, width_mult=1.0, round_nearest=8, cfg_arch=None):
        ...
        # replace stage 4 with a transformer block
        c = _make_divisible(80 * width_mult, round_nearest)
        layer = []
        for i in range(2):
          layer.append(Block(input_channel, input_channel))
        layer.append(Block(input_channel, c))
        layer = nn.Sequential(*layer)
        self.stage.append(layer)
        self.channel.append(c)
    ...
    def forward(self, x):
        ...
        x = x_list[-1].reshape(b, c, h * w).permute(0, 2, 1)
        tmp = self.stage[-1](x)
        ch = tmp.shape[0] * tmp.shape[1] * tmp.shape[2] // (b*h*w)
        x = tmp.permute(0, 2, 1).reshape(b, ch, h, w)
        x_list.append(x)
        final_outputs = []
