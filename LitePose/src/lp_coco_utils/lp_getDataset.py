...
class CrowdPoseKeypoints(CrowdPoseDataset):
    """
     initializes the arguments:
        ...
        whether to remove images without annotations
        generators for heatmaps and joints
        whethrer the transformations should be applied to the images
    """
    def __init__(self,
                 root,
                 dataset_name,
    ...
    def __getitem__(self, idx):
        """
         retrieve an item from the dataset using an index idx
         return  an image, 
                 a list of target heatmaps,
                 a list of masks, and a list of joints
        """
    ...
    def get_joints(self, anno):
        """
        extracts the joint positions from the annotations of an image
        """
        num_people = len(anno)
    ...
    def get_mask(self, anno, idx):
        """
        extracts the mask of an image from its annotations
        """
        coco = self.coco
...
def getDatasetProcessed(split):
    """
    return: an instance of the CrowdPoseKeypoints class->
            the path to the dataset, 
            the split("train", "validation", "trainval","test")
            a boolean value indicating whether images without annotations should be removed
            the hm -- HeatmapGenerator and j -- JointsGenerator lists 
            and the transforms object.
    """
    # checks if the provided split is valid and raises an exception if it is not
    ...
    # apply a random affine transformation to the input images
    # specify the output size:256, rotation angle range, scale factor range
    transforms = T.Compose(
            [
                ...
                # apply a random horizontal flip to the input images
